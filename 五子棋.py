...
class Chess(object):
    ...
    def __init__(self):
        ...
        self.l_info = Label(self.f_header, text="No started", bg=self.header_bg, font=("楷体", 18, "bold"), fg="black")
        self.b_regret = Button(self.f_header, text="Retract", command=self.bf_regret, state=DISABLED, font=self.btn_font)
        self.b_lose = Button(self.f_header, text="Give up", command=self.bf_lose, state=DISABLED, font=self.btn_font)
    ...
    def draw_mesh(self, x, y):
        # 一个倍率，由于tkinter的GUI，如果不加倍率，悔棋是会出现痕迹所以给他加上倍率
        ...
        if ((x == 3 or x == 11) and (y == 3 or y == 11)) or (x == 7 and y == 7):
            self.c_chess.create_oval(center_y - self.point_r, center_x - self.point_r,
                                     center_y + self.point_r, center_x + self.point_r, fill="red")
    ...
    def bf_start(self):
        ...
        self.matrix = [[0 for y in range(self.column)] for x in range(self.row)]
        self.draw_board()
        self.l_info.config(text="黑方选手落子")
    ...
    def bf_lose(self):
        ...
        self.is_start = False
        text = self.ternary_operator("黑方选择认输", "白方选择认输")
        self.l_info.config(text=text)
        self.center_show("你可真是太“蔡”了")
    ...
    def trans_identify(self):
        ...
        text = self.ternary_operator("黑方选手落子", "白方选手落子")
        self.l_info.config(text=text)
